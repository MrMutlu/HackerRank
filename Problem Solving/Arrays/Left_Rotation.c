...
int* rotateLeft(int d, int arr_count, int* arr, int* result_count) {
    *result_count = arr_count;
    // dynamically allocate temporary array to store
    int *temp = (int*)malloc(d * sizeof(int));

    for(int i = 0; i < d; i++)
    {
        // temporary holding indexes in size of d
        temp[i] = arr[i];
    }
    ...
    // copy elements from temp array to end of array
    for(int i = 0; i < d; i++)
    {
        arr[arr_count - d + i] = temp[i];
    }
