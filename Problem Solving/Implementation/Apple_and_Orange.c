...
void countApplesAndOranges(int s, int t, int a, int b, int apples_count, int* apples, int oranges_count, int* oranges) {
    // redefine for easier reading
    int houseStartloc = s;
    int houseEndloc = t;
    int appleTreeloc = a;
    int orangeTreeloc = b;
    int *applesDistance = apples;
    int *orangesDistance = oranges;
    int appleTreeDistance = apples_count;
    int orangeTreeDistance = oranges_count;
    int appleCount = 0, orangeCount = 0;

    for(int i = 0; i < apples_count; i++){
        
        int appleRegion = appleTreeloc + applesDistance[i];
        if( appleRegion >= houseStartloc && appleRegion <= houseEndloc){
            appleCount++;
        }  
    }
    ...
    for(int i = 0; i < oranges_count; i++){
        ...
        int orangeRegion = orangeTreeloc + orangesDistance[i];
        
        if( orangeRegion >= houseStartloc && orangeRegion <= houseEndloc){
            orangeCount++;
        }
    }
